#ifndef EasyTierCore_h
#define EasyTierCore_h
...
// err_msg: Output error message if failed (needs to be freed if not null? Usually err_msg in this pattern is static or allocated. 
// Looking at rust code: CString::new(e).into_raw(). So YES, it needs to be freed by free_string if not null.)
int init_logger(const char* path, const char* level, const char** err_msg);

// Set TUN file descriptor
// Note: On macOS Helper (Root), EasyTier might create TUN directly.
