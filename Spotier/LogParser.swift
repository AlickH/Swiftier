...
class LogParser: ObservableObject {
    ...
    private var logPath: String {
        ...
        return "/var/log/swiftier-helper.log"
    }
    ...
    private func quickParseLogs(_ content: String) -> [LogEntry] {
        ...
        // 获取全局设置级别（从 App Group 读取）
        let settingLevelStr = UserDefaults(suiteName: "group.com.alick.swiftier")?.string(forKey: "logLevel") ?? "INFO"
    ...
    private func readNewRawLines() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if self.isPaused {
                self.pendingLogs.append(contentsOf: parsed)
            } else {
                self.logs.append(contentsOf: parsed)
