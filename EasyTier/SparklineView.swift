...
struct SparklineView: NSViewRepresentable {
    ...
    class Coordinator {
        ...
        deinit {
            ...
            // DispatchQueue.main.async { EasyTierRunner.shared.removeSubscriber() }
        }
    }
}
...
struct SmartSparklineView: View {
    ...
    var body: some View {
        SparklineView(data: data, color: color, maxScale: maxScale, paused: paused)
            .onAppear { EasyTierRunner.shared.addSubscriber() }
            .onDisappear { EasyTierRunner.shared.removeSubscriber() }
    }
}
...
final class SparklineNSView: NSView {
    ...
    func updateData(_ newData: [Double], isLayoutPass: Bool = false) {
        ...
        if isFirstRealUpdate {
            let lastTime = EasyTierRunner.shared.lastDataTime
            let now = Date()
